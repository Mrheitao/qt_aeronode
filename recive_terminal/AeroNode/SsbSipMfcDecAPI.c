    ...
    while (1) {
        while (startCode != USR_DATA_START_CODE) {
            if ((startCode == VOP_START_CODE) || (leng_idx == Frameleng)) {
                printf("[%s] VOP START Found !!.....return",__func__);
              //  printf("[%s] Non Packed PB",__func__);
                return 0;
            }
      ...
      //  printf("[%s] User Data Found !!",__func__);

        do {
            if (*strmBuffer == 'p') {
                printf("[%s] Packed PB",__func__);
                return 1;
            }
            getAByte(strmBuffer, &startCode);
            strmBuffer++; leng_idx++;
...
void *SsbSipMfcDecOpen(void)
{
    ...
    printf("[%s] MFC Library Ver %d.%02d",__func__, MFC_LIB_VER_MAJOR, MFC_LIB_VER_MINOR);
#ifdef CONFIG_MFC_FPS
    framecount = 0;
    over30ms = 0;
    ...
    if (ret != 0) {
        printf("[%s] S_FMT failed",__func__);
        goto error_case2;
    }
    ...
    if (ret != 0) {
        printf("[%s] VIDIOC_REQBUFS failed",__func__);
        goto error_case2;
    }
    ...
    for (i = 0; i < pCTX->v4l2_dec.mfc_num_src_bufs; ++i) {
        ...
        if (ret != 0) {
            printf("[%s] VIDIOC_QUERYBUF failed",__func__);
            goto error_case3;
        }
        ...
        if (pCTX->v4l2_dec.mfc_src_bufs[i] == MAP_FAILED) {
            printf("[%s] mmap failed (%d)",__func__,i);
            goto error_case3;
        }
    }
...
void *SsbSipMfcDecOpenExt(void *value)
{
    ...
    if (NO_CACHE == (*(SSBIP_MFC_BUFFER_TYPE *)value)) {
        ...
        printf("[%s] cacheable buffer",__func__);
    }
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecClose(void *openHandle)
{
    ...
    printf(">>> time=%d", mTS2.tv_sec-mTS1.tv_sec);
    printf(">>> framecount=%d", framecount);
    printf(">>> 30ms over=%d", over30ms);
#endif
    if (openHandle == NULL) {
    ...
    if (pCTX->inter_buff_status & MFC_USE_DST_STREAMON) {
        ...
        if (ret != 0) {
            printf("[%s] VIDIOC_STREAMOFF failed (destination buffers)",__func__);
    ...
    if (pCTX->inter_buff_status & MFC_USE_SRC_STREAMON) {
        ...
        ret = ioctl(pCTX->hMFC, VIDIOC_STREAMOFF, &type);
        if (ret != 0) {
            printf("[%s] VIDIOC_STREAMOFF failed (source buffers)",__func__);
            return MFC_RET_CLOSE_FAIL;
        }
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecInit(void *openHandle, SSBSIP_MFC_CODEC_TYPE codec_type, int Frameleng)
{
    ...
    case VC1RCV_DEC:
        ...
        break;
    default:
        printf("[%s] Does NOT support the standard (%d)",__func__,pCTX->codecType);
    ...
    if (ret != 0) {
        printf("[%s] VIDIOC_STREAMON failed",__func__);
        ret = MFC_RET_DEC_INIT_FAIL;
        goto error_case1;
    }
    ...
    if (ret != 0) {
        printf("[%s] VIDIOC_G_FMT failed",__func__);
        ret = MFC_RET_DEC_INIT_FAIL;
        goto error_case1;
    }
    ...
    if (ret != 0) {
        printf("[%s] VIDIOC_G_CROP failed",__func__);
        ret = MFC_RET_DEC_INIT_FAIL;
        goto error_case1;
    }
    ...
    if (ret != 0) {
        printf("[%s] VIDIOC_G_CTRL failed",__func__);
        ret = MFC_RET_DEC_INIT_FAIL;
        goto error_case1;
    }
    ...
    ret = ioctl(pCTX->hMFC, VIDIOC_REQBUFS, &reqbuf);
    if (ret != 0) {
        printf("[%s] VIDIOC_REQBUFS failed (destination buffers)",__func__);
        ret = MFC_RET_DEC_INIT_FAIL;
        goto error_case1;
    }
    ...
    for (i = 0;  i < pCTX->v4l2_dec.mfc_num_dst_bufs; ++i) {
        ...
        if (ret != 0) {
            printf("[%s] VIDIOC_QUERYBUF failed (destination buffers)",__func__);
            ret = MFC_RET_DEC_INIT_FAIL;
            goto error_case1;
        }
        ...
        if (pCTX->v4l2_dec.mfc_dst_bufs[i][0] == MAP_FAILED) {
            printf("[%s] mmap failed (destination buffers (Y))",__func__);
            ret = MFC_RET_DEC_INIT_FAIL;
            goto error_case2;
        }
        ...
        if (pCTX->v4l2_dec.mfc_dst_bufs[i][1] == MAP_FAILED) {
            printf("[%s] mmap failed (destination buffers (UV))",__func__);
            ret = MFC_RET_DEC_INIT_FAIL;
            goto error_case2;
        }
    ...
    ret = ioctl(pCTX->hMFC, VIDIOC_STREAMON, &type);
    if (ret != 0) {
        printf("[%s] VIDIOC_STREAMON failed (destination buffers)",__func__);
        ret = MFC_RET_DEC_INIT_FAIL;
        goto error_case1;
    }
    pCTX->inter_buff_status |= MFC_USE_DST_STREAMON;
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecExe(void *openHandle, long  lengthBufFill)
{
    _MFCLIB *pCTX;
    int ret;
    struct v4l2_buffer qbuf;
    ...
    if ((lengthBufFill < 0) || (lengthBufFill > MAX_DECODER_INPUT_BUFFER_SIZE)) {
        printf("[%s] lengthBufFill is invalid. (lengthBufFill=%d)",__func__, lengthBufFill);
        return MFC_RET_INVALID_PARAM;
    }
    ...
    if ((lengthBufFill > 0) && (SSBSIP_MFC_LAST_FRAME_PROCESSED != pCTX->lastframe)) {
        ...
        /* wait for decoding */
        do {
            poll_state = poll((struct pollfd*)&poll_events, 1, 25);
...
#if 0
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecExeNb(void *openHandle, long lengthBufFill)
{
    _MFCLIB *pCTX;
    int ret;
...
void  *SsbSipMfcDecGetInBuf(void *openHandle, void **phyInBuf, int inputBufferSize)
{
    ...
    if ((inputBufferSize < 0) || (inputBufferSize > MAX_DECODER_INPUT_BUFFER_SIZE)) {
        printf("[%s] inputBufferSize = %d is invalid",__func__, inputBufferSize);
        return NULL;
    }
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecSetInBuf(void *openHandle, void *phyInBuf, void *virInBuf, int size)
{
    ...
    printf("[%s] Exit idx %d",__func__,pCTX->v4l2_dec.beingUsedIndex);
    return MFC_RET_OK;
}
...
SSBSIP_MFC_DEC_OUTBUF_STATUS SsbSipMfcDecGetOutBuf(void *openHandle, SSBSIP_MFC_DEC_OUTPUT_INFO *output_info)
{
    ...
    case MFC_GETOUTBUF_DISPLAY_END:
        ...
        if (ret) {
            printf("[%s] fail to get secure id(%d) from vaddr(%x)\n",__func__, \
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecSetConfig(void *openHandle, SSBSIP_MFC_DEC_CONF conf_type, void *value)
{
    ...
    if ((value == NULL) && (MFC_DEC_SETCONF_IS_LAST_FRAME !=conf_type)) {
        printf("[%s] value is NULL",__func__);
        return MFC_RET_INVALID_PARAM;
    }
    ...
    case MFC_DEC_SETCONF_FIMV1_WIDTH_HEIGHT: /* be set before calling SsbSipMfcDecInit */
         ...
         printf("fimv1->width  = %d\n", fimv1_res->width);
         printf("fimv1->height = %d\n", fimv1_res->height);
   ...
   //     ctrl.id = V4L2_CID_CODEC_DISPLAY_DELAY;
    ...
    default:
        printf("[%s] conf_type(%d) is NOT supported",__func__, conf_type);
        return MFC_RET_INVALID_PARAM;
    }

    ret = ioctl(pCTX->hMFC, VIDIOC_S_CTRL, &ctrl);
    if (ret != 0) {
        printf("[%s] VIDIOC_S_CTRL failed (conf_type = %d)",__func__, conf_type);
        return MFC_RET_DEC_SET_CONF_FAIL;
    }
...
SSBSIP_MFC_ERROR_CODE SsbSipMfcDecGetConfig(void *openHandle, SSBSIP_MFC_DEC_CONF conf_type, void *value)
{
    ...
    case MFC_DEC_GETCONF_CRC_DATA:
       ...
       // ctrl.id = V4L2_CID_CODEC_CRC_DATA_LUMA;
        ...
        ret = ioctl(pCTX->hMFC, VIDIOC_G_CTRL, &ctrl);
        if (ret != 0) {
            printf("[%s] VIDIOC_G_CTRL failed, V4L2_CID_CODEC_CRC_DATA_LUMA",__func__);
        ...
        ret = ioctl(pCTX->hMFC, VIDIOC_G_CTRL, &ctrl);
        if (ret != 0) {
            printf("[%s] VIDIOC_G_CTRL failed, V4L2_CID_CODEC_CRC_DATA_CHROMA",__func__);
            return MFC_RET_DEC_GET_CONF_FAIL;
        }
        crc_data->chroma0 = ctrl.value;

        printf("[%s] crc_data->luma0=%d",__func__,ctrl.value);
        printf("[%s] crc_data->chroma0=%d",__func__,ctrl.value);
        break;
